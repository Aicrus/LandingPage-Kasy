...
import { REGIONAL_COUPON_CODE } from "@/lib/stripe/regional-discount";
import { cn } from "@/lib/utils";

/** Nota única abaixo dos planos: mensagem + cupom. Só em regiões PPP. */
export function RegionalDiscountNote() {
  const t = useTranslations("pricing.regionalDiscount");
  const [copied, setCopied] = useState(false);

  async function copyCode() {
    try {
      await navigator.clipboard.writeText(REGIONAL_COUPON_CODE);
      setCopied(true);
  ...
  return (
    ...
    >
      ...
      >
        <span className="relative z-10">{REGIONAL_COUPON_CODE}</span>
        {copied ? (
